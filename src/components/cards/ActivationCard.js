        ...
        <Card style={{ width: '21rem', borderRadius: "10px", background: "#4d4d4d" }}>
            <Card.Body>
                ...
                <Card.Text>
                    ...
                    {p.role === 'user' && <p style={{ display: "flex", color: "#fff" }}>
                    ...
                    {user.role === 'admin' && p.role === "seller" && <p style={{ display: "flex", color: "#fff" }}>Package Name: {p.packageId && (p.packageId.name).toUpperCase()}</p>}
                    {user.role === 'admin' && p.role === "seller" && <p style={{ display: "flex", color: "#fff" }}>Remaining Days: {p.remainingDays && p.remainingDays}</p>}
                    {user.role === 'admin' && p.role === "seller" && <p style={{ display: "flex", color: "#fff" }}>Remaining Products: {p.remainingProducts && p.remainingProducts}</p>}
                </Card.Text>
